            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2">Edit Aturan</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-md sm:rounded-lg">
                <div class="p-6 md:p-8">
                    <h3 class="text-2xl font-bold text-gray-800 mb-2">Edit Aturan Alur Kerja</h3>
                    <p class="text-gray-500 mb-6">Perbarui alur persetujuan berdasarkan level jabatan pengguna.</p>

                    <form action="{{ route('admin.workflows.update', $workflow->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <select name="requester_level_id" id="requester_level_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                                    @foreach($levels as $level)
                                        <option value="{{ $level->id }}" {{ $workflow->requester_level_id == $level->id ? 'selected' : '' }}>
                                            {{ $level->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="approver_level_id" id="approver_level_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                                    @foreach($levels as $level)
                                        <option value="{{ $level->id }}" {{ $workflow->approver_level_id == $level->id ? 'selected' : '' }}>
                                            {{ $level->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="scope" id="scope" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                                    <option value="department" {{ $workflow->scope == 'department' ? 'selected' : '' }}>Departemen yang Sama</option>
                                    <option value="subsidiary" {{ $workflow->scope == 'subsidiary' ? 'selected' : '' }}>Subsidiary yang Sama</option>
                                </select>
                                <p class="mt-2 text-xs text-gray-500">Aturan ini hanya berlaku dalam lingkup yang sama.</p>
                            </div>
