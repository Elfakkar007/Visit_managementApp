        ...
        <div class="relative p-4 w-full max-w-4xl h-auto">
            <div class="relative bg-white rounded-lg shadow" x-data="{ activeTab: 'profile' }">
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
                    <h3 class="text-xl font-semibold text-gray-900">{{ $editingId ? 'Edit Pengguna' : 'Tambah Pengguna Baru' }}</h3>
                ...
                <form wire:submit.prevent="save">
                    <div class="p-4 md:p-5">
                        <div class="mb-4 border-b border-gray-200">
                            <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" role="tablist">
                                <li class="me-2" role="presentation">
                                    <button @click.prevent="activeTab = 'profile'" :class="{ 'text-blue-600 border-blue-600': activeTab === 'profile', 'border-transparent hover:text-gray-600 hover:border-gray-300': activeTab !== 'profile' }" class="inline-block p-4 border-b-2 rounded-t-lg" type="button" role="tab">Profil & Peran</button>
                                </li>
                                <li class="me-2" role="presentation">
                                    <button @click.prevent="activeTab = 'permissions'" :class="{ 'text-blue-600 border-blue-600': activeTab === 'permissions', 'border-transparent hover:text-gray-600 hover:border-gray-300': activeTab !== 'permissions' }" class="inline-block p-4 border-b-2 rounded-t-lg" type="button" role="tab">Izin Khusus</button>
                                </li>
                            </ul>
                        </div>

                        {{-- KONTEN TAB PROFIL & PERAN --}}
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="space-y-4">
                                    <div>
                                        ...
                                        <select wire:model="subsidiary_id" id="subsidiary_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required><option value="">Pilih...</option>@foreach($subsidiaries as $s)<option value="{{ $s->id }}">{{ $s->name }}</option>@endforeach</select>
                                    ...
                                    <div>
                                        ...
                                        <select wire:model="department_id" id="department_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required><option value="">Pilih...</option>@foreach($departments as $d)<option value="{{ $d->id }}">{{ $d->name }}</option>@endforeach</select>
                        ...
                        {{-- KONTEN TAB IZIN KHUSUS --}}
                        <div x-show="activeTab === 'permissions'" style="display: none;">
                            <div class="p-3 bg-gray-50 border border-gray-300 rounded-lg max-h-64 overflow-y-auto">
                               <p class="text-sm text-gray-600 mb-3">Berikan izin tambahan yang tidak termasuk dalam peran default pengguna.</p>
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-2 gap-x-4">
                                    @forelse($all_permissions as $permission)
                                        <div class="flex items-center">
                                            <input wire:model="assigned_direct_permissions" type="checkbox" value="{{ $permission->name }}" id="perm_{{ $permission->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                                            <label for="perm_{{ $permission->id }}" class="ms-2 text-sm font-medium text-gray-900">{{ $permission->name }}</label>
                                        </div>
                                    @empty
                                        <p class="text-sm text-gray-500">Tidak ada izin ditemukan.</p>
                                    @endforelse
                                </div>
                            </div>
                            @error('assigned_direct_permissions') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>
                    </div>
        ...
        <div class="relative p-4 w-full max-w-3xl h-auto">
            <div class="relative bg-white rounded-lg shadow" x-data="{ activeTab: 'profile' }">
                ...
                <div class="p-4 md:p-5">
                    <div class="mb-4 border-b border-gray-200">
                        <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" role="tablist">
                            <li class="me-2" role="presentation"><button @click.prevent="activeTab = 'profile'" :class="{ 'text-blue-600 border-blue-600': activeTab === 'profile' }" class="inline-block p-4 border-b-2 rounded-t-lg" type="button" role="tab">Profil</button></li>
                            <li class="me-2" role="presentation"><button @click.prevent="activeTab = 'permissions'" :class="{ 'text-blue-600 border-blue-600': activeTab === 'permissions' }" class="inline-block p-4 border-b-2 rounded-t-lg" type="button" role="tab">Izin & Aktivitas</button></li>
                        </ul>
                    </div>
                    <div>
                        ...
                        <div x-show="activeTab === 'permissions'" style="display: none;" class="p-4 rounded-lg bg-gray-50 space-y-4">
                             <div>
                                <h4 class="font-medium text-gray-800">Peran Default</h4>
                                <p class="text-sm text-gray-600">{{ $detailUser->getRoleNames()->implode(', ') ?: 'Tidak ada peran.' }}</p>
                            </div>
                            <div>
                                <h4 class="font-medium text-gray-800">Izin Khusus (Direct)</h4>
                                <p class="text-sm text-gray-600">{{ $detailUser->getDirectPermissions()->pluck('name')->implode(', ') ?: 'Tidak ada izin khusus.' }}</p>
                            </div>
                            <hr/>
                            <div>
                                <h4 class="font-medium text-gray-800">5 Aktivitas Terakhir</h4>
                                <ul class="divide-y divide-gray-200">
                                    @forelse($detailUser->visitRequests as $request)
                                        <li class="py-2"><p class="text-sm text-gray-600">Tujuan: {{ $request->destination }} ({{ $request->created_at->format('d M Y') }})</p></li>
                                    @empty
                                        <li class="py-2 text-sm text-gray-500">Tidak ada aktivitas.</li>
                                    @endforelse
                                </ul>
                            </div>
                        </div>
                ...
                <div class="flex items-center justify-end p-4 md:p-5 border-t border-gray-200 rounded-b">
                    <button wire:click="closeModal" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700">Tutup</button>
                </div>
            </div>
        </div>
    </div>
